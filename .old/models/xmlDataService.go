...
type XMLRollCallVoteResults struct {
	...
	Titles  []XMLTitle  `xml:"RollCallVoteResults.Titles>RollCallVoteResults.Title.Text"`
...
func (x *XMLRollCallVoteResults) Dump() {
	fmt.Printf("%+v\n", x)
}
...
func (s *XMLDataService) IngestData(rawXMLData []byte) error {
	...
	//create corresponding objects from model to each xml element
